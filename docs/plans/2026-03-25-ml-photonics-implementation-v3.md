# ML Photonics Surrogate & Generative Engine — Implementation Plan v3
...
describe('graph power propagation', () => {
  ...
  it('beam splitter distributes power to two outputs', () => {
    ...
    // 50:50 split with 0.3 dB insertion loss
    // Each arm: 10 - 3.01 (split) - 0.3 (loss) ≈ 6.7 dBm
    // Total output power preserved (in linear): should roughly equal input minus loss
  });
});
...
4. Apply validation to builds routes:
```typescript
import { insertBuildSchema } from '@workspace/db';
import { validateBody } from '../middleware/validate.js';

// POST /builds
router.post('/', validateBody(insertBuildSchema), async (req, res) => { /* ... */ });

// PUT /builds/:id
router.put('/:id', validateBody(insertBuildSchema.partial()), async (req, res) => { /* ... */ });
```
...
- **Per-node head:** Linear(128 → 64 → 4) predicting [outputPower, loss, phase, status]
...
async function runInference(graph: GraphInput): Promise<PredictionOutput> {
  // ... ONNX tensor creation and session.run()
}
...
3. Add warm-up call to `artifacts/api-server/src/index.ts` after server starts:
```typescript
// After server.listen():
